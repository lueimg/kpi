    ...
    dbQuery('select 1 from UBIGEO where name = ? and codprov = 0 and coddist = 0 and status = 1', [name],
      function (err, rows) {
        ...
        if (rows.length === 0) {
          ...
          dbQuery('SELECT max(coddpto) dpto from UBIGEO', function (err, rows) {
            if (err) {
              printLog(err);
              res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
              return;
            ...
            dbQuery('insert into UBIGEO SET ? ', zone, function (err) {
              if (err) {
                printLog(err);
                res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
                return;
...
rutas.route('/Departamento/:id')
  .get(function (req, res) {
    ...
    dbQuery('select * from UBIGEO where coddpto = ?  and status = 1', [id], function (error, rows) {
      if (error) {
        printLog(error);
  ...
  .delete(function (req, res) {
    ...
    dbQuery('update UBIGEO set status = "0" where coddpto = ?', [coddpto], function (err) {
      if (err) {
        printLog(err);
        res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
        return;
  ...
  .post(function (req, res) {
    ...
    dbQuery('select 1 from UBIGEO where ' +
      ...
      'name = ? and status = 1;', [dpto, name], function (err, rows) {
        ...
        if (rows.length === 0) {
          // Generate new code ubigeo
          dbQuery('SELECT IFNULL(max(codprov),0) id from UBIGEO  where coddpto = ?', [dpto], function (err, rows) {
            var cDpto,
                cProv,
                cDist;
    ...
    dbQuery('select 1 from UBIGEO where ' +
      ...
      'name = ? and status = 1;', [dpto, prov, id, name], function (err, rows) {
        ...
        if (rows.length === 0) {
          ...
          dbQuery('update UBIGEO SET ? where coddpto = ? and codprov = ? and coddist = ?', [zone, dpto, prov, id],
            function (err) {
              if (err) {
                printLog(err);
                res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
