var rutas = require('express').Router(),
    responseUtils = require('../core/ResponseUtils'),
...
rutas.route('/')
  ...
  .post(function (req, res) {
    ...
    db.beginTransaction(function (err) {
      ...
      dbQuery('select * from users where username = ?', [usernameLowerCase], function (err, rows) {
        var query,
            userClientQuery = [],
...
rutas.route('/:id')
  .get(function (req, res) {
    ...
    dbQuery(query + CQuery + RQuery, function (err, results) {
      if (err) {
        printLog(err);
        res.status(500).send({code: 500, message: 'Internal Server Error', dev: err});
        return;
