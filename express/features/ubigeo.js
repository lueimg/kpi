...
routes.route('/list')
  ...
  .post(function (req, res) {
    ...
    dbQuery('INSERT INTO UBIGEO SET ?;', ubigeo, function (err, result) {
      if (err) {
        printLog(err);
        res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
      }
  ...
  .put(function (req, res) {
    ...
    dbQuery('UPDATE UBIGEO SET ? WHERE ID = ?;', [ubigeo, ubigeo.id],
      function (err) {
        if (err) {
          printLog(err);
          res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
  ...
  .delete(function (req, res) {
    ...
    dbQuery('UPDATE UBIGEO SET ? WHERE ID = ?;', [ubigeo, ubigeo.id], function (err) {
      if (err) {
        printLog(err);
        res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
      }
...
routes.route('/')
  .get(function (req, res) {
    ...
    dbQuery(dataQuery + where, function (err, rows) {
      if (err) {
        printLog(err);
  ...
  .post(function (req, res) {
    ...
    dbQuery('select 1 from ZONES where ?', {name: zone.name}, function (err, rows) {
      if (err) {
        printLog(err);
        res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
        return;
      ...
      if (rows.length === 0) {
        dbQuery('Insert into ZONES SET ?', zone, function (err) {
          if (err) {
            printLog(err);
            res.status(500).send({code: 500, msg: 'Internal Server Error', dev: err});
            return;
